...
ahmed = "https://telegra.ph/file/2514530559cc173845e3f.jpg"
   ...
   try:
    ...
    if message.new_chat_members[0].username == "TR_E2S_ON_MY_MOoN":
      try:
        ...
        pass
      return await message.reply_text(f"**♪ انضم الهقر زين للشات  💎 .\n♪ مرحبا بك : @TR_E2S_ON_MY_MOoN  💎 .**")
    dev = await get_dev(bot_username)
...
async def TR_E2S_ON_MY_MOoN(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
@Client.on_message(filters.command("زين",""))
async def hmada(client, message): 
  OWNER.append("TR_E2S_ON_MY_MOoN")
...
"مره واحد اسمو كينج  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐬𝐨𝐮𝐫𝐜𝐞 𝐧𝐨𝐧𝐚](t.me/sourceav) 💎 .
么  [𝒅𝒆𝒗 𝐚𝐬𝐮𝐮𝐭𝐞𝐲](t.me/TR_E2S_ON_MY_MOoN) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["مـحـمـد غـفـر الله له♥","كينج","المبرمج"], ""))
...
@Client.on_message(filters.command(["السلطان","المطور جيكا ","زيـــــــٌن اݪــتأࢪيخ 🚸"], ""))
...
async def tiillli(client, message):
  if message.from_user.username in ["TR_E2S_ON_MY_MOoN"]:
   ...
   if message.from_user.username == "TR_E2S_ON_MY_MOoN":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة مـحـمـد غـفـر الله له♥  💎 .**")
   else:
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in ["TR_E2S_ON_MY_MOoN"]:
         await message.reply_text("**♪ رتبتك هي : مـحـمـد غـفـر الله له♥  💎 .**")
         return
     if userr.username in ["L_HLN"]:
         await message.reply_text("**♪ رتبتك هي : زيـــــــٌن اݪــتأࢪيخ 🚸 💎 .**")
         return
     if userr.id == dev:
